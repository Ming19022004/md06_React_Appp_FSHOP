...
export default function CheckoutScreen({ route, navigation }: any) {
  ...
  const calculateVoucherDiscount = (subtotalAmount: number, shippingFeeAmount: number) => {
    if (!appliedVoucher) return 0;
    let discount = 0;
    const percent = Number(appliedVoucher.discount_percent) || 0;
    const fixedAmount = Number(appliedVoucher.discount_amount || appliedVoucher.value) || 0;
        ...
        <View style={styles.itemContent}>
          ...
          <View style={styles.itemDetails}>
            <View>
              ...
              <Text style={styles.detailText}>SL: <Text style={styles.detailValue}>{item.quantity}</Text></Text>
            </View>
            <View style={styles.priceContainer}>
              <Text style={styles.price}>{finalPrice.toLocaleString()} đ</Text>
              {hasDiscount && <Text style={styles.originalPrice}>{product.price.toLocaleString()} đ</Text>}
            </View>
          </View>
        </View>
      </View>
      ...
      >
        ...
        <View style={styles.voucherCenter}>
            ...
            <Text style={styles.voucherDescList}>
                {item.discount_percent ? `Giảm ${item.discount_percent}%` : `Giảm ${Number(item.discount_amount||item.value||0).toLocaleString()}đ`}
            </Text>
        </View>
          ...
          <View style={styles.container}>
            ...
            <View style={styles.section}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('PersonalInfo')}>
                <Text style={styles.editLink}><Icon name="create-outline" size={14} color={PRIMARY} /> Chỉnh sửa</Text>
              </TouchableOpacity>
            </View>
              ...
              {['COD', 'Online'].map(method => (
                ...
                >
                  <View style={styles.paymentContent}>
                    ...
                    <Text style={styles.paymentText}>{method === 'COD' ? 'Thanh toán khi nhận hàng' : 'Thanh toán Online'}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.footerContainer}>
            ...
            <View style={styles.totalSection}>
              ...
              <View style={[styles.totalRow, styles.finalTotal]}>
                ...
                <Text style={styles.finalAmount}>{(Number(total) || 0).toLocaleString()} đ</Text>
              </View>
            </View>
            ...
            <View style={styles.modalContent}>
                ...
                {loadingVouchers ? (
                    ...
                    <FlatList 
                        ...
                        ListEmptyComponent={<Text style={styles.emptyText}>Không có mã giảm giá nào.</Text>}
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', height: 56, paddingHorizontal: 12, backgroundColor: PRIMARY },
  backIcon: { padding: 4, width: 44, height: 44, justifyContent: 'center', alignItems: 'center' },
  ...
  section: { marginBottom: 16, backgroundColor: '#fff', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: BORDER_COLOR },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 10 },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: '#1a1a1a' },
  addressText: { fontSize: 14, color: '#555', lineHeight: 22, marginBottom: 10 },
  editLink: { color: PRIMARY, fontSize: 14, fontWeight: '600', marginTop: 8 },
  itemContainer: { flexDirection: 'row', backgroundColor: '#fff', padding: 12, marginHorizontal: 16, marginBottom: 10, borderRadius: 12, borderWidth: 1, borderColor: BORDER_COLOR },
  image: { width: 90, height: 90, borderRadius: 12, marginRight: 12, backgroundColor: LIGHT_BG, borderWidth: 1, borderColor: BORDER_COLOR },
  itemContent: { flex: 1, justifyContent: 'space-between', gap: 8 },
  name: { fontSize: 14, fontWeight: '700', color: '#1a1a1a', lineHeight: 20 },
  itemDetails: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  detailText: { fontSize: 12, color: '#666', fontWeight: '500', marginBottom: 4 },
  ...
  paymentButton: { borderWidth: 1.5, borderColor: BORDER_COLOR, padding: 12, borderRadius: 10, marginTop: 10, backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center' },
  ...
  radioButton: { width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: BORDER_COLOR, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff' },
  ...
  appliedVoucher: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f0fdf4', borderWidth: 1, borderColor: '#dcfce7', borderRadius: 10, padding: 12, marginTop: 10 },
  ...
  divider: { height: 1, backgroundColor: BORDER_COLOR, marginHorizontal: 16, marginBottom: 16 },
  footerContainer: { backgroundColor: '#fff', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 24, borderTopWidth: 1, borderTopColor: BORDER_COLOR },
  totalSection: { marginBottom: 18 },
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: BORDER_COLOR },
  ...
  confirmButton: { backgroundColor: PRIMARY, paddingVertical: 16, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 10, shadowColor: PRIMARY, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 5 },
