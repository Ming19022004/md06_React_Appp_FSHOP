...
const OrderTrackingScreen = () => {
  ...
  const calculateDisplayPrice = (item: OrderItem) => {
    ...
    const subTotal = item.items.reduce((sum, prod) => {
        return sum + (Number(prod.price) || 0) * (Number(prod.purchaseQuantity) || 1);
    }, 0);

    // Phí ship (mặc định 30k nếu null)
    const ship = item.shippingFee !== undefined ? Number(item.shippingFee) : 30000;
    
    // Giảm giá
    const discount = Number(item.voucherDiscount) || 0;
    ...
    if (!final || final === 0 || (discount > 0 && final === subTotal + ship)) {
  ...
  const handleCancelOrder = async (orderId: string) => {
    try {
      ...
      Alert.alert('Lỗi', err.response?.data?.message || 'Không thể huỷ đơn hàng lúc này.');
    }
  };
  ...
  const renderItem = ({ item }: { item: OrderItem }) => {
    ...
    const isCancellable = ['waiting', 'pending'].includes((item.status || '').toLowerCase());
      ...
      <Pressable onPress={() => setSelectedOrder(item)} style={styles.orderBox}>
        <View style={{ flex: 1 }}>
          <View style={styles.orderHeaderRow}>
             ...
             <Text style={{ color: getStatusColor(item.status), fontWeight: '600', fontSize: 13 }}>
          ...
          {item.items.map((product, idx) => (
            <View key={idx} style={styles.productRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{fontSize: 12, color: '#666'}}>x{product.purchaseQuantity}</Text>
          ...
          <View style={styles.totalContainer}>
             ...
             {discount > 0 && (
                 <View style={styles.discountRow}>
                    ...
                    <Text style={styles.discountText}>Voucher giảm: -{discount.toLocaleString()}đ</Text>
                 </View>
             )}
          ...
          <View style={styles.actionRow}>
            ...
            {item.status === 'delivered' && (
                <TouchableOpacity
                    onPress={() => navigation.navigate('ReviewScreen', { orderId: item._id, products: item.items })}
  ...
  const renderModal = () => {
    ...
    const { subTotal, shippingFee, discount, finalTotal } = calculateDisplayPrice(selectedOrder);
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.infoSection}>
                  <Text style={styles.modalLabel}>Mã đơn: <Text style={styles.valueText}>#{selectedOrder.order_code || selectedOrder._id}</Text></Text>
                  <Text style={styles.modalLabel}>Trạng thái: <Text style={{ color: getStatusColor(selectedOrder.status), fontWeight: 'bold' }}>{translateStatus(selectedOrder.status)}</Text></Text>
                  <Text style={styles.modalLabel}>Ngày đặt: <Text style={styles.valueText}>{formatDate(selectedOrder.createdAt)}</Text></Text>
                  <Text style={styles.modalLabel}>Thanh toán: <Text style={styles.valueText}>{selectedOrder.paymentMethod?.toUpperCase()}</Text></Text>
                  <Text style={styles.modalLabel}>Địa chỉ: <Text style={styles.valueText}>{selectedOrder.shippingAddress}</Text></Text>
              </View>

              <Text style={styles.sectionHeader}>Sản phẩm</Text>
              {selectedOrder.items.map((item, index) => (
                <View key={index} style={styles.productItemContainer}>
                    <Text style={styles.productItemName}>{index + 1}. {item.name}</Text>
                    <View style={styles.productItemRow}>
                        <Text style={styles.productItemQty}>x{item.purchaseQuantity}</Text>
                        <Text style={styles.productItemPrice}>{Number(item.price).toLocaleString()}đ</Text>
                    </View>
                </View>
              ))}
              ...
              <View style={styles.paymentDetail}>
                  ...
                  <View style={styles.paymentRow}>
                      ...
                      <Text style={styles.paymentValue}>{shippingFee.toLocaleString()}đ</Text>
                  </View>
                  {discount > 0 && (
                    <View style={styles.paymentRow}>
                        <Text style={[styles.paymentLabel, {color: GREEN}]}>Voucher giảm giá:</Text>
                        <Text style={[styles.paymentValue, {color: GREEN}]}>-{discount.toLocaleString()}đ</Text>
                    </View>
                  )}
                  <View style={[styles.paymentRow, styles.finalRow]}>
                      <Text style={styles.finalLabel}>Tổng thanh toán:</Text>
                      <Text style={styles.finalValue}>{finalTotal.toLocaleString()}đ</Text>
                  </View>
              </View>
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, paddingHorizontal: 12, paddingTop: 8, backgroundColor: '#F8FAFB' },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 52, marginBottom: 12, backgroundColor: PRIMARY, borderRadius: 8 },
  backIcon: { position: 'absolute', left: 12, padding: 8 },
  headerTitle: { fontSize: 20, fontWeight: 'bold', color: '#fff' },
  
  orderBox: { backgroundColor: '#fff', borderRadius: 12, padding: 12, marginBottom: 12, elevation: 2 },
  ...
  discountRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 2 },
  ...
  actionRow: { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 4, gap: 10 },
  cancelBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, backgroundColor: '#fee2e2' },
  cancelBtnText: { color: RED, fontWeight: '600', fontSize: 13 },
  reviewBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, backgroundColor: ORANGE },
  ...
  sectionHeader: { fontSize: 16, fontWeight: '700', marginBottom: 10, color: '#1a1a1a' },
  productItemContainer: { marginBottom: 10, borderBottomWidth: 1, borderBottomColor: '#f1f1f1', paddingBottom: 8 },
  ...
  finalRow: { marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#eee' },
  ...
  tabItem: { paddingVertical: 8, paddingHorizontal: 14, marginRight: 10, borderRadius: 20, backgroundColor: '#fff', borderWidth: 1, borderColor: '#f1f5f9' },
